...
<strong>Вопросы с выбором ответа (поле «Ответ»):</strong><br>
Если в поле «Ответ» указаны варианты в специальном формате, студенту показываются не поле ввода, а выбор одного или нескольких вариантов. Каждая строка — один вариант, первый символ строки — маркер:<br>
• <code>-</code> — вариант ответа (неправильный или один из вариантов)<br>
• <code>*</code> — правильный вариант при <strong>множественном выборе</strong> (несколько правильных, отображаются чекбоксы)<br>
• <code>#</code> — единственный правильный вариант при <strong>одном выборе</strong> (отображаются радиокнопки)<br><br>

<strong>Пример одного правильного ответа (один выбор):</strong><br>
<pre style="background: #f5f5f5; padding: 10px; border-radius: 4px; margin: 4px 0;">
- Берлин
- Мадрид
# Париж
- Рим
</pre>
В интерфейсе студент увидит радиокнопки; правильный ответ — «Париж».<br><br>

<strong>Пример нескольких правильных ответов (множественный выбор):</strong><br>
<pre style="background: #f5f5f5; padding: 10px; border-radius: 4px; margin: 4px 0;">
- Ответ 1
* Ответ 2
* Ответ 3
...
В интерфейсе студент увидит чекбоксы; правильный ответ — выбор вариантов 2 и 3.<br><br>

<strong>Ограничения:</strong> нельзя смешивать <code>#</code> и <code>*</code>; для одного выбора допускается только одна строка с <code>#</code>. Если нужен свободный ввод или проверка по регулярному выражению, оформите ответ без этих маркеров (обычный текст или regex).<br><br>
...
• Флаг проверяется регистронезависимо и работает как альтернатива обычному ответу (для заданий без формата выбора)<br><br>

<strong>Использование:</strong><br>
Пользователи видят вопросы на веб-странице и могут вводить ответ или выбирать варианты. Система проверяет обычный ответ (или выбор по индексам для вариантов), а также флаг. Задания могут быть зачтены через API, не только через ввод ответа на вебе.<br><br>
"""
}
